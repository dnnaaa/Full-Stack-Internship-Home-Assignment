...
public class PdfConfig extends PdfPageEventHelper {
	...
	public void onEndPage(PdfWriter writer, Document document) {
		try {
			if (pagination) {
				...
				if(baseFont == null){
					setBaseFont(BaseFont.createFont(BaseFont.HELVETICA, BaseFont.CP1252, BaseFont.NOT_EMBEDDED));
				}
	...
	public void addExportDate(Document document) throws Exception {
		...
		PdfPCell cell = new PdfPCell(new Phrase("Exporté le : " + Utils.dateTimeToString(LocalDateTime.now()), font));
