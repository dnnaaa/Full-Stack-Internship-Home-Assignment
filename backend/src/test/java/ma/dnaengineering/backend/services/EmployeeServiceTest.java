...
class EmployeeServiceTest {
    ...
    void testProcessCsvFile() throws IOException {
        ...
        csvData.add(new String[]{"1", "Azeddine", "Doe", "2000.0"});


        try (MockedStatic<Csv> utilities = Mockito.mockStatic(Csv.class)) {
            utilities.when(() -> Csv.parseCsv(any())).thenReturn(csvData);
    ...
    void testCalculateAverageSalaryByJobTitle() {
        List<Employee> employees =
                List.of(Employee.builder().id(1L).name("Azeddine").jobTitle("Engineer").salary(5000.0).build(),
